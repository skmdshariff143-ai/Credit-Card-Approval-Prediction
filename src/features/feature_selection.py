    ...
    def fit_selection(self, X: pd.DataFrame, y: pd.Series):
        ...
        try:
            ...
            rf = RandomForestClassifier(
                n_estimators=100, max_depth=12, class_weight="balanced", random_state=RANDOM_STATE, n_jobs=-1
            )
            rf.fit(X, y)
            ...
            self.importances_df = (
                pd.DataFrame({"Feature": X.columns, "RF_Importance": rf_importances, "Mutual_Information": mi_scores})
                .sort_values(by="RF_Importance", ascending=False)
                .reset_index(drop=True)
            )

            # 4. Filter features based on RF importance threshold
            self.selected_features = self.importances_df[self.importances_df["RF_Importance"] >= self.threshold][
                "Feature"
            ].tolist()
