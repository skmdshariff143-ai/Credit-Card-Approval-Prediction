    ...
    def process_and_predict(self, form_data: dict) -> dict:
        ...
        try:
            ...
            raw_data = {
                ...
                "DAYS_EMPLOYED": (
                    365243
                    if form_data.get("flag_unemployed")
                    else -int(float(form_data.get("years_employed", 0)) * 365.25)
                ),
                "FLAG_MOBIL": 1,
            ...
            history.add_entry(
                input_data=form_data, decision=result["decision"], probability=result["approval_probability_percent"]
            )
