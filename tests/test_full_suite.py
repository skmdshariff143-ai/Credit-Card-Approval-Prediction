...
from src.utils.helper import save_pkl, load_pkl, save_json, load_json
from src.utils.exceptions import DataLoadingError, DataPreprocessingError, DataValidationError, ModelTrainingError
from src.visualization.eda import generate_summary_stats, calculate_missing_matrix
...
def mock_app_df():
    return pd.DataFrame({
        ...
        "AMT_INCOME_TOTAL": [120000.0, 90000.0, 300000.0, -150000.0, 200000.0, 180000.0, 150000.0, 250000.0, 350000.0, 110000.0],
        "NAME_INCOME_TYPE": ["Working", "Commercial associate", "Pensioner", "Working", "State servant", "Working", "Commercial associate", "Pensioner", "Working", "Working"],
        "NAME_EDUCATION_TYPE": ["Secondary / secondary special", "Higher education", "Incomplete higher", "Secondary / secondary special", "Higher education", "Secondary / secondary special", "Higher education", "Secondary / secondary special", "Higher education", "Secondary / secondary special"],
        "NAME_FAMILY_STATUS": ["Married", "Single / not married", "Civil marriage", "Married", "Single / not married", "Married", "Single / not married", "Civil marriage", "Married", "Single / not married"],
        "NAME_HOUSING_TYPE": ["House / apartment", "House / apartment", "Rented apartment", "House / apartment", "House / apartment", "House / apartment", "House / apartment", "Rented apartment", "House / apartment", "House / apartment"],
        ...
        "OCCUPATION_TYPE": ["Laborers", np.nan, np.nan, "Core staff", "Accountants", "Laborers", np.nan, np.nan, "Core staff", "Laborers"],
...
def test_model_registry(tmp_path):
    with patch.object(config, 'get_paths', return_value={"models_dir": str(tmp_path)}):
        ...
        y = pd.Series([0, 1, 0, 1])
        trainer = ModelTrainer()
        model, _ = trainer.train_and_time_model("logistic_regression", trainer.get_baseline_models()["logistic_regression"], X, y)
...
def test_main_pipeline_run(mock_cv_score, mock_savefig, tmp_path):
    ...
    with patch.object(config, 'get_paths', return_value=mock_paths):
        with patch('src.main.ModelTrainer.get_baseline_models', return_value={"logistic_regression": LogisticRegression()}):
...
def test_predict_methods_and_wrappers(mock_exists, mock_load):
    from src.models.predict import RiskPredictor, load_pipeline, load_model, validate_input, predict, predict_probability
    ...
    valid = validate_input({
        ...
        'name_income_type': 'Working', 'name_education_type': 'Secondary', 
        'name_family_status': 'Married', 'name_housing_type': 'House', 
        'years_employed': 5.0, 'flag_unemployed': 0
    })
